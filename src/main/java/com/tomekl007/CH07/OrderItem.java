...
public class OrderItem {
  ...
  /**
   * Computes the final date on which this item can be returned in the simple "click a button"
   * workflow. This is base on the date on which the item is shipped from the warehouse, from the
   * perspective of the delivery location. The returns period (currently three months; see {@link
   * #RETURNS_PERIOD}) is added to the shipping date to obtain the final returns date. When adding
   * the returns period, if the day-of-month goes beyond the end of the resulting month, the result
   * should be the start of the following month.
   *
  ...
  static LocalDate getFinalReturnsDate(Instant shippingTime, ZoneId destinationTimeZone) {
    LocalDate shippingDateAtDestination = shippingTime.atZone(destinationTimeZone).toLocalDate();
